...
import {
  ...
  GitBranch, Layers, RefreshCw, Server,
...
export interface DevTabContentProps {
  tab: DevTab;
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function DevTabContent({ tab, t }: DevTabContentProps) {
  const [flags, setFlags] = useState(FEATURE_FLAGS_DEFAULT);
              ...
              <motion.div key={agent.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-white/3 rounded-2xl border border-white/8 p-5">
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  {[
                    ...
                    { label: "Errors",   value: String(agent.errors),           color: agent.errors > 10 ? "text-red-400" : "text-white/60" },
                  ].map((s) => (
