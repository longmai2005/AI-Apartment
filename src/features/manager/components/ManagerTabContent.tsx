...
import {
  Users, Wrench, Zap, KeyRound, DollarSign, Star,
  Search, Filter, PlusCircle, ChevronRight, MoreHorizontal,
  CheckCircle2, XCircle, AlertTriangle, Bell,
  Eye, Edit3, Phone, Trash2, Send, Download,
  Car, Droplets, Wifi, User,
  TrendingUp, BarChart3, ArrowUpRight, ArrowDownRight,
  Settings as SettingsIcon,
} from "lucide-react";
import { useCountUp } from "@shared/hooks/useCountUp";

export type ManagerTab = "dashboard" | "residents" | "requests" | "utilities" | "access" | "announcements" | "finance" | "settings";

export interface ManagerTabContentProps {
  tab: ManagerTab;
  setTab: (t: ManagerTab) => void;
  search: string;
  setSearch: (s: string) => void;
  t: (vi: string, en: string) => string;
  session: { name?: string; email?: string; building?: string };
}
...
const FINANCE = {
  income: [
    { month: "T1", rent: 98, service: 12, parking: 8 }, { month: "T2", rent: 98, service: 12, parking: 8 },
    { month: "T3", rent: 101, service: 13, parking: 9 }, { month: "T4", rent: 105, service: 13, parking: 9 },
  ],
  expenses: [
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <motion.div whileHover={{ rotate: 10, scale: 1.1 }} transition={{ type: "spring", stiffness: 400, damping: 20 }}
...
// ─── ManagerTabContent ────────────────────────────────────────────────────────
export default function ManagerTabContent({ tab, setTab, search, setSearch, t, session }: ManagerTabContentProps) {
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white/4 rounded-2xl p-5 border border-white/6">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-amber-400" style={{ fontSize: "0.78rem" }}><AlertTriangle size={13} /><span>{t("1 hợp đồng hết hạn", "1 contract expired")}</span></div>
                    <div className="flex items-center gap-2 text-red-400"   style={{ fontSize: "0.78rem" }}><XCircle size={13} /><span>{t("1 yêu cầu khẩn cấp", "1 urgent request")}</span></div>
                    <div className="flex items-center gap-2 text-blue-400"  style={{ fontSize: "0.78rem" }}><Bell size={13} /><span>{t("Cắt điện ngày 05/05", "Power cut 05/05")}</span></div>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <tr key={r.id} className="border-b border-white/4 hover:bg-white/3 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400 font-semibold" style={{ fontSize: "0.8rem" }}>{r.name[0]}</div>
                          <div>
                ...
                <tbody>
                  {UTILITIES.map((u) => (
                    <tr key={u.unit} className="border-b border-white/4 hover:bg-white/3 transition-colors">
                      ...
                      <td className="px-4 py-3"><span className="text-emerald-400" style={{ fontSize: "0.78rem" }}>{u.elevStatus}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div className="divide-y divide-white/4">
                {ACCESS_LOG.map((log, i) => (
                  <div key={i} className="flex items-center gap-4 px-5 py-3.5 hover:bg-white/3 transition-colors">
                    ...
                    <span className={`px-2.5 py-0.5 rounded-full border text-xs flex-shrink-0 ${statusBadge[log.status]}`}>{log.status === "allowed" ? t("Cho phép","Allowed") : t("Từ chối","Denied")}</span>
                    <div className="flex-1">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className={`px-2.5 py-0.5 rounded-full border text-xs ${statusBadge[a.status]}`}>{a.status === "published" ? t("Đã đăng","Published") : t("Nháp","Draft")}</span>
                      </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-4 pt-2">
                    {[{c:"bg-emerald-500",l:t("Tiền thuê","Rent")},{c:"bg-blue-500",l:t("Dịch vụ","Service")},{c:"bg-violet-500",l:t("Bãi xe","Parking")}].map(s=>(
                      <div key={s.l} className="flex items-center gap-1.5" style={{ fontSize: "0.7rem" }}><div className={`w-2 h-2 rounded-full ${s.c}`} /><span className="text-white/40">{s.l}</span></div>
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="max-w-xl space-y-4">
            <div className="bg-white/4 rounded-2xl border border-white/6 p-5">
              ...
              <div className="flex items-center gap-4 mb-5">
                <div className="w-14 h-14 rounded-2xl bg-emerald-500/20 flex items-center justify-center text-emerald-400 text-xl font-bold">{session.name?.[0] || "M"}</div>
                <div>
